...
export default function PromoCodeCopyButton({
  ...
  const statusMessage =
    copyState === "copied"
      ...
      : copyState === "error"
        ? "Unable to copy the promo code. Please try again."
        : "";
  ...
  return (
    <button
      ...
      className={`group absolute z-10 flex h-[clamp(2rem,7vw,3.25rem)] w-[54%] -translate-x-1/2 -translate-y-1/2 cursor-pointer items-center overflow-hidden rounded-full bg-[linear-gradient(105deg,#55a5fb_0%,#397de9_46%,#315bd4_100%)] px-[clamp(0.625rem,2.2vw,1rem)] text-white shadow-[0_8px_22px_rgba(38,83,177,0.2),inset_0_1px_0_rgba(255,255,255,0.38)] transition-transform duration-500 ease-[cubic-bezier(0.32,0.72,0,1)] hover:scale-[1.015] active:scale-[0.985] focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#173f9f] focus-visible:ring-offset-2 ${className}`}
      onClick={handleCopy}
    >
      <span
        aria-hidden="true"
      ...
      <span className="flex min-w-0 flex-1 items-baseline justify-center gap-[0.28em] whitespace-nowrap text-[clamp(0.69rem,2.9vw,1.35rem)] leading-none tracking-[-0.02em]">
        ...
        <span className="font-semibold tracking-[-0.015em]">[{code}]</span>
      </span>
